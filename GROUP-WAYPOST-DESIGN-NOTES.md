# Group Waypost Design Notes

## Problem Statement

Add a new waypost capability that behaves like a group address / lightweight group chat:
...
Current waypost semantics are centered on two objects:

- `message`: the content record.
- `delivery`: one delivery to one recipient endpoint, with state such as `queued`, `leased`, `acked`, `failed`.

This works well for personal queue / queue semantics, but group chat requirements do not fit the existing model cleanly because:
...
Treat personal waypost and group waypost as two different semantics:

- Personal waypost: queue semantics.
- Group waypost: append-only message stream plus per-person read state.
...
Keep existing personal waypost behavior unchanged.

Recommended split:

- existing queue-oriented commands continue to work for personal queue addresses
