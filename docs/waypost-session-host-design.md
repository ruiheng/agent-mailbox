...
## Agent Deck Parent-Group Snapshot

This behavior does not change the generic `session_create` request or result
schemas, Thurbox behavior, legacy Agent Deck tools, Agentgear, or durable
delivery semantics.
...
No group-list probe, second launch, post-create move, fallback launch, or Agent
Deck version probe is permitted. Thurbox argv and all legacy group-placement
